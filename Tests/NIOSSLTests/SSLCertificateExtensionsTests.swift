...
final class SSLCertificateExtensionsTests: XCTestCase {
    func test() throws {
        ...
        let basicConstraint = try XCTUnwrap(cert._extensions.first(where: { $0.objectIdentifier == .init("2.5.29.19")! }))
        let subjectKeyIdentifier = try XCTUnwrap(cert._extensions.first(where: { $0.objectIdentifier == .init("2.5.29.14")! }))
        let authorityKeyIdentifier = try XCTUnwrap(cert._extensions.first(where: { $0.objectIdentifier == .init("2.5.29.35")! }))
    ...
    func testEmptyExtensions() {
        let extensions = _SSLCertificateExtensions()
    ...
    func testUnowned() throws {
        ...
        extensions = nil
        XCTAssertNil(weakReferenceToOwner, "extensions are released and therefore no one should still have a strong reference to the owner")
    }
}
